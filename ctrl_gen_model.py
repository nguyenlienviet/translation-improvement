...
class CtrlGenModel(object):
    ...
    def _build_model(self, inputs, vocab, gamma, lambda_g):
        ...
        enc_text_ids = inputs['source_text_ids'][:, 1:]
        enc_outputs, final_state = encoder(embedder(enc_text_ids),
                                           sequence_length=inputs['source_length'] - 1)

        # Teacher-force decoding and the auto-encoding loss for G
        decoder = AttentionRNNDecoder(
            memory=enc_outputs,
            memory_sequence_length=inputs['source_length'] - 1,
        ...
        g_outputs, _, _ = decoder(
            initial_state=connector(final_state), inputs=inputs['source_text_ids'],
            embedding=embedder, sequence_length=inputs['source_length'] - 1)

        loss_g_ae = tx.losses.sequence_sparse_softmax_cross_entropy(
            labels=inputs['source_text_ids'][:, 1:],
        ...
        start_tokens = tf.fill([tf.shape(inputs['source_text_ids'])[0]], 1) * vocab.bos_token_id
        end_token = vocab.eos_token_id
        gumbel_helper = GumbelSoftmaxEmbeddingHelper(
            embedder.embedding, start_tokens, end_token, gamma)

        soft_outputs_, _, soft_length_, = decoder(
            helper=gumbel_helper, initial_state=connector(final_state))

        # Greedy decoding, used in eval
        outputs_, _, length_ = decoder(
            decoding_strategy='infer_greedy', initial_state=connector(final_state),
        ...
        labels_target = tf.fill([tf.shape(inputs['target_text_ids'])[0]], 1.0)
        clas_logits, clas_preds = classifier(
            inputs=clas_embedder(ids=inputs['target_text_ids'][:, 1:]),
            sequence_length=inputs['target_length'] - 1)
        loss_d_clas = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=labels_target, logits=clas_logits)
        loss_d_clas = tf.reduce_mean(loss_d_clas)
        accu_d = tx.evals.accuracy(labels=labels_target, preds=clas_preds)

        # Classification loss for the generator, based on soft samples
        labels_source = tf.fill([tf.shape(inputs['source_text_ids'])[0]], 0.0)
        ...
        accu_g = tx.evals.accuracy(labels=labels_source,
                                   preds=soft_preds)
        ...
        self.samples = {
            "original": inputs['source_text_ids'][:, 1:],
        ...
        fetches_eval = {"batch_size": get_batch_size(inputs['source_text_ids'])}
